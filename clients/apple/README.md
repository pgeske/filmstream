...
Long cold starts distinguish Finding a Release from Buffering Video instead of showing one indefinite preparation state. Finding a Release covers playback creation; Buffering Video covers subtitle discovery and server HLS startup. A successful HLS response means the server has packaged its initial buffer, not that AVPlayer has displayed video. The player keeps a bounded liveness budget until its video layer is ready for display and player time advances. tvOS permits one automatic reconnect per uninterrupted stall, without resetting the existing 45-second failure deadline on an HTTP response, item replacement, or programmatic pause; iOS and Mac retain their bounded failure behavior without automatic source replacement. Explicit pause stops this budget. Terminal errors remain visible rather than being cleared by a late callback or Play on the same failed item.

Detail and episode screens own their preparation request, cancel it when navigation abandons the screen, and reject late results before presentation. Autoplay work belongs to the current player and is canceled on dismissal. Item-changing seeks, subtitle rendition changes, and tvOS recovery share a generation fence; stopped or superseded work cannot install an item or complete a seek. Item failure/end notifications are accepted only for the currently installed item. Local `make apple-test` fixtures cover preparation cancellation/re-entry, late HLS results, cancellation-safe cleanup, bounded source retry, and startup/midplay liveness. These deterministic tests exercise the shared policy and request ownership, not actual AVPlayer rendering, PiP, or physical Apple TV playback. Detail pages show only IMDb and Rotten Tomatoes scores; TMDB votes remain catalog-ranking metadata rather than user-facing ratings. Top Rated requires substantially more votes so established movies and shows are favored over obscure new entries.
